/**
 * Accurate Provider Excel Converter
 * Properly extracts all provider data from structured sheets
 */
...
providerSheets.forEach((sheetName, index) => {
  try {
    ...
    const statesCol = columns.find(c => c.includes('States Licensed') || c.includes('License') || c.includes('Licenses'));
    const licenseCol = columns.find(c => (c.includes('LICENSE') || c.includes('License')) && !c.includes('States'));
    ...
    data.forEach(row => {
      ...
      // Extract full name from NAME column (better than sheet name)
      if (nameValue && nameValue.length > 5 && !nameValue.includes('NPI') && 
          /^[A-Z]/.test(nameValue) && !nameValue.match(/^\d+/) && 
          !nameValue.includes('@') && !nameValue.includes('phone')) {
        const cleanName = nameValue
          .replace(/NPI[:\s#]*\d{10}/gi, '')
          .replace(/\s*-\s*(NP|MD|DO|RN|FNP|FNP-C).*$/i, '')
          .trim();
        ...
        if (cleanName && cleanName.length > 3) {
          const nameParts = cleanName.split(/\s+/).filter(p => 
            p.length > 0 && 
            !p.match(/^\d+$/) && 
            !p.match(/^(St|Ave|Rd|Dr|Ln|Way|Apt|Suite)/i)
          );
      ...
      if (statesValue && statesValue.includes('DEA') && licenseValue) {
        // DEA format: 2 letters + 7 digits (e.g., MD9419777)
        const deaMatch = licenseValue.match(/([A-Z]{2}\d{7})/);
        if (deaMatch) {
          provider.dea = deaMatch[1];
        } else if (licenseValue && licenseValue.length > 5 && !licenseValue.includes('DEA')) {
      ...
      if (nameValue && /\d+\s+[A-Z][a-z]+/.test(nameValue) && 
          (nameValue.includes('St') || nameValue.includes('Ave') || nameValue.includes('Rd') || 
           ...
           nameValue.includes('Apt') || nameValue.includes('Suite'))) {
        const addressMatch = nameValue.match(/(\d+\s+[A-Z][a-z\s]+(?:St|Ave|Rd|Dr|Ln|Way|Blvd|Ct|Pl)[^@]*)/i);
        if (addressMatch && !provider.address) {
          provider.address = addressMatch[1].trim().split('\n')[0];
        }
      }
      
      // Extract email and phone from all columns
...
console.log('\n📋 Review the JSON file. The data is ready to bundle with your extension!');
